...
import { InstagramIcon, FacebookIcon, GithubIcon, LinkedInIcon, TwitterIcon, SunIcon, MoonIcon } from './Icon'
...
const NavBar = () => {
  ...
  return (
    <header className='w-full px-20 py-5 font-medium flex items-center justify-between
     dark:text-light relative z-10 lg:px-16 md:px-12 sm-px-8'>
        <button className='flex-col justify-center items-center hidden lg:flex' onClick={handleClick}>
            <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out  h-1 w-6 rounded-sm  ${isOpen ? 'rotate-45 translate-y-1' : '-translate-y-0.5'}`}>  </span>
            <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out  h-1 w-6 rounded-sm  my-0.5 ${isOpen ? 'opacity-0' : 'opacity-100'}`}>  </span>
            <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out  h-1 w-6 rounded-sm  ${isOpen ? '-rotate-45 -translate-y-1' : 'translate-y-0.5'}`}>  </span>
        </button>
        ...
        <nav className='flex items-center justify-center flex-wrap mt-2'>
            ...
            whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 mx-3 bg-light rounded-full dark:bg-dark sm:mx-1' > <InstagramIcon/> </motion.a>
            <motion.a href={"https://www.facebook.com/star.shangeeth.7?mibextid=ZbWKwL"} target='_blank' 
            whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 mx-3 bg-light rounded-full dark:bg-dark sm:mx-1' > <FacebookIcon/> </motion.a>
             <motion.a href={"/"} target='_blank'
            whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 ml-3 sm:mx-1' > <TwitterIcon/> </motion.a>
        ...
        <div className='absolute left-[50%] top-2 translate-x-[50%] sm:translate-x-0 sm:left-[40%] '>
        <Logo/>
        </div>
    </header>
  )
